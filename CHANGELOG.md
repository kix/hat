...
- **📺 Большой экран / Режим ТВ (TV Spectator Mode):**
  - Специализированный полноэкранный интерфейс для телевизоров, проекторов и совместной игры в гостиной (подключение через URL `?tv=CODE` или кнопку «📺 Режим ТВ» в лобби).
  - **Честная игра для зрителей:** загаданное слово скрыто на большом экране, чтобы гости в комнате могли отгадывать вместе с игроками, пока объясняющий видит слово на своём смартфоне.
  - Огромный анимированный таймер с пульсацией, живой счёт команд, анонс следующей пары, вспышки серий и торжественный финал.
- **✨ AI-генератор тематических наборов слов (Custom AI Pack Generator):**
  - Генерация любого тематического словаря по запросу (по Гарри Поттеру, IT и стартапам, сериалу «Офис», медицине, мемам, аниме, кино и др.).
  - Быстрые тематические подсказки, выбор размера пака (20, 30, 50 слов), сложности и языка (RU/EN).
  - Интерактивный предпросмотр с возможностью редактирования, добавления и удаления слов перед добавлением в шляпу.
  - Поддержка как прямого подключения к Gemini API через пользовательский ключ, так и офлайн семантического генератора.
- **🏆 Турнирная сетка плей-офф (Tournament Bracket Mode):**
  - Полноценный режим чемпионата на 4 или 8 команд для больших вечеринок и турниров.
  - Интерактивное дерево турнирной сетки (1/4 финала → 1/2 финала → Матч за 3-е место → Гранд-Финал).
  - Автоматический запуск матчей и продвижение победителей в следующий раунд.
  - Сохранение состояния турнира в `localStorage` и церемония награждения на пьедестале почёта (🥇 🥈 🥉) с конфетти.

## [1.14.0] - 2026-09-23

### Added

- **🎩 Классический трёхэтапный режим игры («Слова → Жесты → Одно слово» / Classic 3-Stage Hat Mode):**
  - Возможность играть по каноничным турнирным правилам Шляпы в 3 последовательных этапа с одним и тем же пулом слов:
    1. **🗣️ Этап 1: Словами** — классическое объяснение словами без однокоренных выражений.
    2. **🎭 Этап 2: Пантомима (Жесты)** — показ исключительно жестами и мимикой, без звуков и предметов.
    3. **☝️ Этап 3: Одно слово** — подсказка состоит ровно из одного ассоциативного слова.
  - **🔄 Бесшовные переходы между этапами:**
    - Когда шляпа пустеет на 1-м и 2-м этапе, все слова из исходного банка автоматически возвращаются в шляпу, перемешиваются заново, и игра переходит к экрану анонса следующего этапа с правилами.
    - Очки и статистика накапливаются сквозным образом на протяжении всех 3 этапов.
  - **🏷️ Наглядные бейджи и подсказки:**
    - Индикаторы текущего этапа (🗣️ / 🎭 / ☝️) и напоминания правил на экранах анонса хода и игрового процесса.
  - **📊 Итоговая разбивка очков по этапам:**
    - На финальном экране игры добавлена подробная матрица результатов каждой команды / игрока по всем трем этапам отдельно.
  - **⚙️ Полная совместимость:**
    - Режим работает со всеми форматами участников: Командный, Парный и Личный зачёт («Каждый с каждым»).
